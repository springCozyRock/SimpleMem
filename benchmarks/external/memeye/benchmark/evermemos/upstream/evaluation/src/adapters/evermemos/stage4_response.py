...
def _memcell_atomic_fact_text(memcell: Dict[str, dict]) -> str:
    atomic_fact = memcell.get("atomic_fact")
...
def _memcell_episode_text(memcell: Dict[str, dict]) -> str:
    episode = str(memcell.get("episode", "") or "").strip()
    if episode:
        return episode
    return _memcell_atomic_fact_text(memcell)


def _memcell_subject_text(memcell: Dict[str, dict]) -> str:
    subject = str(memcell.get("subject", "") or "").strip()
    if subject:
        return subject
    atomic_text = _memcell_atomic_fact_text(memcell)
    if not atomic_text:
        return "N/A"
...
def build_context_from_event_ids(
    ...
    for event_id in selected_event_ids:
        ...
        subject = _memcell_subject_text(memcell)
        episode = _memcell_episode_text(memcell) or "N/A"
