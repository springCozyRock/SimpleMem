...
# Model names that use OpenAI-compatible API (OpenAI or DashScope compatible-mode)
...
_DASHSCOPE_TEXT_EMBEDDING_MODELS = {
    "text-embedding-v4",
    "text-embedding-v3",
    "text-embedding-v2",
    "text-embedding-v1",
...
class EmbeddingService:
    ...
    def _is_api_text_embedding_model(self, model_name: str) -> bool:
        if self._is_openai_model(model_name):
            return True
        if model_name in _DASHSCOPE_TEXT_EMBEDDING_MODELS:
            return True
    ...
    def _detect_text_backend(self) -> str:
        ...
        if self._is_api_text_embedding_model(model):
            return "openai"
    ...
    def _get_openai_client(self):
        ...
        if self._openai_client is None:
            ...
            import httpx
            client_kwargs = {}
            api_key = getattr(self.embedding_config, "api_key", None) or self.config.llm.api_key
        ...
        if self._local_text_model is None:
            with self._embedding_lock:
                if self._local_text_model is None:
                    ...
                    if self._is_api_text_embedding_model(model_name):
                        model_name = self.LOCAL_TEXT_MODEL
                    st_kwargs = {}
                    device = os.environ.get("SIMPLEMEM_EMBEDDING_DEVICE", "").strip()
                    if device:
                        st_kwargs["device"] = device
                    self._local_text_model = SentenceTransformer(model_name, **st_kwargs)
                    logger.info(
                        f"Loaded local text embedding model: {model_name}"
