...
class Entity:
    ...
    def merge(self, other: "Entity") -> None:
        ...
        self_name = (self.name or "").strip()
        if other_name and other_name.lower() != self_name.lower():
            if other_name not in self.aliases:
                self.aliases.append(other_name)

        for alias in other.aliases:
            alias_text = str(alias or "").strip()
            if not alias_text:
                continue
            if alias_text not in self.aliases and alias_text.lower() != self_name.lower():
                self.aliases.append(alias_text)
...
class KnowledgeGraph:
    ...
    def add_entity(self, entity: Entity) -> str:
        ...
        if not (entity.name or "").strip():
            logger.debug("Skipping entity with empty name")
        ...
        with self._lock:
            for entity in self._entities.values():
                ...
                for alias in entity.aliases:
                    ...
                    if alias_text and query_lower in alias_text:
                        results.append(entity)
                        break
